...
class WallpaperGUI:
    ...
    def create_menu(self):
        ...
        view_menu.add_command(label="Previous Image", command=self.previous_image, accelerator="Left Arrow")
        view_menu.add_command(label="Next Image", command=self.next_image, accelerator="Right Arrow")
    ...
    def create_image_display_panel(self, parent):
        ...
        self.image_label = ttk.Label(image_container, text="Select an image to preview", anchor=tk.CENTER)
    ...
    def create_tags_panel(self, parent):
        ...
        add_button = ttk.Button(entry_frame, text="Add", command=self.on_add_tag, width=8)
        add_button.pack(side=tk.RIGHT, padx=(5, 0))

        remove_button = ttk.Button(add_tag_frame, text="Remove Selected Tag", command=self.on_remove_tag)
    ...
    def on_add_tag(self, event=None):
        ...
        try:
            if self.db_manager.add_tag_to_image(image_id, tag_name):
                ...
                messagebox.showinfo("Tag Exists", f"Tag '{tag_name}' already exists for this image.")
        except Exception as e:
    ...
    def on_remove_tag(self):
        ...
        try:
            if self.db_manager.remove_tag_from_image(image_id, tag_name):
                ...
                messagebox.showinfo("Tag Not Found", f"Tag '{tag_name}' was not found for this image.")
        except Exception as e:
    ...
    def show_about(self):
        ...
        info_label = ttk.Label(main_frame, text=info_text, justify=tk.CENTER, font=("Arial", 10))
        info_label.pack(pady=(0, 20))
        
        ttk.Label(main_frame, text="License:", font=("Arial", 10, "bold")).pack(anchor=tk.W)
        
        license_frame = ttk.Frame(main_frame)
        license_frame.pack(fill=tk.BOTH, expand=True, pady=(5, 20))
        
        license_text = tk.Text(license_frame, wrap=tk.WORD, height=10, width=50, font=("Arial", 9))
